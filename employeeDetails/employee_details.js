...
function findSpecialization() {
  const javaScript = employees.find(
    (employee) => employee.specialization === 'Javascript'
  );
  if (javaScript) {
    document.getElementById(
      'employeesDetails'
    ).innerHTML = `<p>${javaScript.id}: ${javaScript.name}: ${javaScript.name} - ${javaScript.department} - $${javaScript.salary}</p>`;
  } else {
    document.getElementById('employeesDetails').innerHTML =
      'no employee has been found in this specialization';
  }
}
